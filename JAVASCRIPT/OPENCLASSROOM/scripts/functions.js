...
function updateMotUtilisateur(listeMots, i) {
    //Modification de la variable motUtilisateur
    motUtilisateur = prompt("Entrez le mot " + listeMots[i]);
    return motUtilisateur;
}

function testMotUtilisateur(motUtilisateur, listeMots, score) {
    // structure conditionnelle 1 pour savoir si motUtilisateur est identique à listeMots[i] et incrémentation de la variable score
    if (motUtilisateur === listeMots[i]) {
        score += 1
        ComparaisonMotUtilisateur = "bien joué ! Votre score est de " + score
    } else {
        ComparaisonMotUtilisateur = "Vous avez fait une faute de frappe, vous avez écrit " + motUtilisateur + ". Votre score est de " + score
    }
    return ComparaisonMotUtilisateur;
}



while (i <= listeMots.length) {
    //Fonction mot utilsateur
    ResultUpdateMotUtilisateur = updateMotUtilisateur()
    console.log(ResultUpdateMotUtilisateur)

    //Fonction Test du mot utilsateur
    ResultTestMotUtilisateur = testMotUtilisateur()
    console.log(ResultTestMotUtilisateur)
